...
// ─── Localhost bypass (for self-hosted local dev) ───────────────────────────
// When the proxy is run locally by a single developer, requiring a login flow
// just to use the dashboard is friction with no security benefit — the socket
// is already bound to 127.0.0.1. This bypass auto-elevates loopback callers
// to admin so `git clone && node server.mjs` works with zero account setup.
//
// Disabled when env DISABLE_LOCALHOST_BYPASS=1, or whenever the request shows
// any sign of having come through a reverse proxy (X-Forwarded-* headers).
// Production deploys behind Caddy/nginx should set DISABLE_LOCALHOST_BYPASS=1.
export const LOCALHOST_BYPASS_ENABLED =
  !["1", "true", "yes"].includes(String(process.env.DISABLE_LOCALHOST_BYPASS || "").toLowerCase());

const LOOPBACK_ADDRS = new Set(["127.0.0.1", "::1", "::ffff:127.0.0.1"]);

export function isLoopbackRequest(req) {
  if (!LOCALHOST_BYPASS_ENABLED) return false;
  // If a reverse proxy forwarded this request, it's not a true loopback caller.
...
// Synthetic admin context returned by checkAdmin for loopback callers.
const LOCALHOST_ADMIN_ROW = Object.freeze({
  key_hash: null,
  name: "localhost",
  role: "admin",
  status: "active",
...
// Random per-process token used by /admin/models/:id/test to call back into
// the proxy (localhost) without needing a real user/admin api key. Never
// exposed externally — only the in-process admin handler uses it.
const INTERNAL_TEST_TOKEN = "sk-proxy-internal-" + randomBytes(24).toString("hex");
export function getInternalTestToken() { return INTERNAL_TEST_TOKEN; }
...
export function checkApiKey(req, returnName) {
  ...
  if (candidate && candidate === INTERNAL_TEST_TOKEN) {
    return returnName ? { valid: true, name: "__internal_test__", tokenName: null, keyRow: null } : true;
  }
  if (candidate) {
 ...
 *  Loopback callers (127.0.0.1 / ::1, no X-Forwarded-* headers) are auto-elevated
 *  unless DISABLE_LOCALHOST_BYPASS=1. See LOCALHOST_BYPASS_ENABLED. */
export function checkAdmin(req) {
  if (isLoopbackRequest(req)) {
    return { ok: true, keyRow: LOCALHOST_ADMIN_ROW, source: "localhost_bypass" };
  }
  const candidate = extractCandidate(req);
