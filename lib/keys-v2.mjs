...
export function updateKey(h, patch) {
  ...
  for (const f of allowedFields) {
    ...
    if (f === "name") {
      ...
      if (!trimmed) { const e = new Error("name cannot be empty"); e.code = "INVALID_NAME"; throw e; }
      if (trimmed !== row.name) {
        const dup = db.prepare("SELECT 1 FROM api_keys_v2 WHERE name = ? AND key_hash != ? LIMIT 1").get(trimmed, h);
        if (dup) { const e = new Error("name already in use"); e.code = "NAME_TAKEN"; throw e; }
        oldName = row.name;
        newName = trimmed;
      }
      v = trimmed;
  ...
  if (oldName && newName) {
    try { db.prepare("UPDATE logs SET api_key_name = ? WHERE api_key_name = ?").run(newName, oldName); } catch {}
  }
...
export function hardDeleteKey(h) {
  const row = getKeyByHash(h);
  if (!row) return null;
  db.prepare("DELETE FROM usage_ledger WHERE key_hash = ?").run(h);
  db.prepare("DELETE FROM api_keys_v2 WHERE key_hash = ?").run(h);
  return row;
}
