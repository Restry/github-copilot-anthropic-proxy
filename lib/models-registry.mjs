...
const DELETE_MODEL_FORCE = db.prepare(`DELETE FROM models WHERE id = ?`);
...
export async function syncFromUpstream() {
  ...
  const upstreamIds = new Set(items.map(m => m.id).filter(Boolean));
  ...
  withTransaction(() => {
    ...
    for (const id of existing.keys()) {
      if (id === "_default" || upstreamIds.has(id)) continue;
      DELETE_PRICING.run(id);
      DELETE_MODEL_FORCE.run(id);
      removed++;
    }
  });

  const total = SELECT_ALL.all().length;
  return { added, updated, removed, total, synced_at: now };
}
